...
products/
  ...
  jet-lighter.html            หน้าขาย "ไฟฟู่ M.T.T." (ตะกร้า + ฟอร์มที่อยู่ + ส่งออเดอร์ทาง LINE)
...
assets/
  ...
  js/shop-config.js           ⭐ ตั้งค่าร้านที่เดียว: LINE, เบอร์, ที่อยู่, บัญชีธนาคาร
...
| `BANK_ACCOUNT` | บัญชีธนาคารสำรอง (แสดงพร้อมปุ่มคัดลอก) |

> ℹ️ เว็บไม่แสดง QR/PromptPay — เจ้าของร้านตัดออกเพราะกลัวยอดโอนที่ระบุตัวผู้โอนไม่ได้ ทีมงานแจ้งวิธีชำระเงินในแชท LINE หลังยืนยันยอด
...
npm test          # โหลดทุกหน้า เช็ค JS error, ตะกร้า, สลับภาษา, จำค่า
```
...
- สรุปออเดอร์คัดลอกได้ในคลิกเดียว ส่งต่อทาง LINE เพื่อยืนยันยอดและค่าส่ง
...
- ข้อมูลจริงใน `shop-config.js` (LINE / เบอร์โทร / ที่อยู่)
